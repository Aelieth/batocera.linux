...
Official Batocera: remote **`upstream`** = `https://github.com/batocera-linux/batocera.linux.git`.  
Aelieth `origin/master` often lags official. Sync this branch with:
...
Do **not** merge the whole official tree into Aelieth `master` without asking. Cherry-pick or merge onto `snes-hd_initial`.  
